...
function checkJump(heights) {
  ...
  const conditionLeft = left.slice(1).every((x, i) => x >= left[i]);
  const conditionRigth = heights.slice(1).every((x, i) => x <= heights[i]);

  return conditionLeft && conditionRigth && !!left.length && heights.length > 1;
}

console.log(checkJump([1, 3, 4, 5, 8, 5, 2]));
