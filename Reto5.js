...
function getMaxGifts(giftsCities, maxGifts, maxCities) {
  const cities = giftsCities.filter((city) => city < maxGifts);
  let iteraciones = [];

  iteraciones.push([], [cities[0]]);

  cities.shift();

  for (let i = 0; i < cities.length; i++) {
    let nuevaLista = iteraciones.map((iteracion) => {
      let _iteraciones = [...iteracion];
      if (_iteraciones.length < maxCities) {
        _iteraciones.push(cities[i]);
      }
      return _iteraciones;
    });
    iteraciones = [...iteraciones, ...nuevaLista];
  }
  iteraciones.shift();
